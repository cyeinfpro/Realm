...
def _schedule_apply_pool(node: Dict[str, Any], pool: Dict[str, Any]) -> None:
    ...
    兼容点：
    - 在有 running loop 的协程上下文：直接 create_task
    - 在无 running loop 的线程/同步上下文：尝试 run_coroutine_threadsafe
    """
    try:
        loop = asyncio.get_running_loop()
        loop.create_task(_bg_apply_pool(node, pool))
        return
    except RuntimeError:
    ...
    try:
        ...
        asyncio.run_coroutine_threadsafe(_bg_apply_pool(node, pool), loop)
    except Exception:
        return
...
async def api_netmon_monitors_update(monitor_id: int, request: Request, user: str = Depends(require_login)):
    ...
    return {
        ...
        "monitor": {
            ...
            "node_ids": [nid for nid in (_safe_int_list(mon2.get("node_ids") or []) ) if nid > 0],
...
async def api_intranet_tunnel_save(payload: Dict[str, Any], user: str = Depends(require_login)):
    ...
    def _norm_host(h: str) -> str:
        ...
        host, _port, _has_port, _scheme = _split_host_and_port(h, 0)
        return host
